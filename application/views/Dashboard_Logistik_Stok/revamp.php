...
<form method="post" id="revamp_form_tambah_stok"
    ...
    <input type="hidden" name="confirm_sj_gap" id="revamp_confirm_sj_gap" value="0">
                    ...
                    <div class="stock-modal__section">
                        ...
                        <div class="row">
                            <div class="col-md-4">
                                ...
                                <small class="text-muted stock-hidden d-block mt-1" id="revamp_nomor_surat_jalan_preview"></small>
                            ...
                            <div class="col-md-12 mt-3 stock-hidden" id="revamp_pabrik_receive_panel">
                                <div class="stock-empty text-left">
                                    <strong>IN dari Pabrik</strong><br>
                                    Masukkan nomor surat jalan pabrik, bukan nomor PO. Jika surat jalan cocok, sistem akan menarik qty pengiriman otomatis dari histori pengiriman pabrik untuk gudang tujuan ini.
                                </div>
                            </div>
                            <div class="col-md-6 mt-3 stock-hidden" id="revamp_ba_selisih_wrapper">
                                <label for="revamp_nomor_ba_selisih">Nomor BA Selisih / Rusak</label>
                                <input type="text" class="form-control" name="nomor_ba_selisih"
                                    id="revamp_nomor_ba_selisih" autocomplete="off"
                                    placeholder="BA/LOG/SELISIH/05/2026">
                            </div>
                        </div>
                    </div>
                        ...
                        <div class="row">
                            <div class="col-md-12">
                                <div class="stock-material-picker">
                                    ...
                                    <small class="text-danger stock-hidden d-block mt-1" id="revamp_qty_validation_feedback"></small>
                                </div>
                            </div>
                                ...
                                <table class="table stock-table" id="revamp_table_item_stok">
                                    <thead>
                                        <tr>
                                            <th>No</th>
                                            <th>Nama</th>
                                            <th id="revamp_table_qty_primary_label">Qty</th>
                                            <th id="revamp_table_qty_secondary_label" class="stock-hidden">Qty Diterima</th>
                                            <th>Qty Selisih</th>
                                            <th>Satuan</th>
                                            <th>Merk Item</th>
                                            <th>No Haspel</th>
            ...
            <div class="modal-body">
                <div class="stock-modal__section">
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-4 mt-3 stock-hidden" id="revamp_detail_pic_pengambil_wrapper">
                            <label>PIC Pengambil</label>
                            <input type="text" class="form-control" id="revamp_detail_nama_mitra" disabled>
                        </div>
                        <div class="col-md-4 mt-3 stock-hidden" id="revamp_detail_telp_pengambil_wrapper">
                            <label>Telp Pengambil</label>
                            <input type="text" class="form-control" id="revamp_detail_pic_mitra" disabled>
                        </div>
                        <div class="col-md-4 mt-3 stock-hidden" id="revamp_detail_nopol_wrapper">
                            <label>NOPOL Kendaraan</label>
                            <input type="text" class="form-control" id="revamp_detail_nomor_polisi" disabled>
                        </div>
                    </div>
                </div>
...
<script>
    (function() {
        ...
        let autoSjPreviewRequest = null;
        let autoSjPreviewReady = false;
        let autoSjLastSuggested = '';
        ...
        function isHoTransferMode() {
            return String($('#revamp_id_sumber_material').val() || '') === '10';
        }
        ...
        function shouldUseManualAutoOutProjectFlow() {
            return isOutProjectMode() && isAutoGeneratedSjMode() && !isShipmentReceiptMode() && !isMitraReturnMode();
        }
        ...
        function isOutMaterialMode() {
            return String($('#revamp_id_sumber_material option:selected').text() || '').toUpperCase().includes('OUT');
        }

        function getOutStockProjectionState() {
            const projection = {};
            let hasNegativeStock = false;
            const messages = [];

            if (!isOutMaterialMode() || isShipmentReceiptMode() || isMitraReturnMode()) {
                return {
            ...
            $('#revamp_table_item_stok tbody tr').each(function() {
                const $row = $(this);
                const idKodeItem = String($row.find('input[name^="id_kode_item["]').val() || '').trim();
                const qtyValue = parseNumber($row.find('input[name^="jumlah_stok["]').val() || 0);

                if (!idKodeItem) {
                    return;
                }

                if (!projection[idKodeItem]) {
                    projection[idKodeItem] = {
                        itemLabel: String($row.data('itemLabel') || 'Material').trim(),
                        availableStock: parseNumber($row.data('availableStock') || 0),
                        requestedQty: 0
                    };
                }

                projection[idKodeItem].requestedQty += qtyValue;
            });

            $.each(projection, function(_, item) {
                const sisaStok = Number(item.availableStock || 0) - Number(item.requestedQty || 0);
                if (sisaStok < 0) {
                    hasNegativeStock = true;
                    messages.push(`${item.itemLabel}: stok tersedia ${formatNumber(item.availableStock || 0)}, request ${formatNumber(item.requestedQty || 0)}, hasil ${formatNumber(sisaStok)}`);
                }
            });
        ...
        function applyOutStockGuard() {
            const state = getOutStockProjectionState();
            const hasBlockingPreview = isAutoGeneratedSjMode() && !isShipmentReceiptMode() && !autoSjPreviewReady;

            $('#revamp_qty_validation_feedback')
                .toggleClass('stock-hidden', !state.hasNegativeStock)
                .html(state.hasNegativeStock ? `Qty membuat stok minus.<br>${state.messages.join('<br>')}` : '');

            if (state.hasNegativeStock || hasBlockingPreview) {
        ...
        function initTambahModal() {
            ...
            function isPabrikReceiptMode() {
                return String($('#revamp_id_sumber_material').val() || '') === sumberMaterialPabrikId;
            }
            ...
            function setAutoSjPreview(message, isError) {
                const text = String(message || '').trim();
                autoSjPreviewReady = !!text && !isError;
                $('#revamp_nomor_surat_jalan_preview')
                    .toggleClass('stock-hidden', !text)
                    .toggleClass('text-danger', !!isError)
                    .toggleClass('text-muted', !isError)
                    .text(text);
            }

            function loadAutoGeneratedSjPreview() {
                if (!isAutoGeneratedSjMode() || isShipmentReceiptMode()) {
                    setAutoSjPreview('', false);
                    return;
                }

                const idGudang = $('#revamp_id_lokasi_gudang').val();
                const tanggalSurat = $('#revamp_tanggal_upload_stok').val();

                if (autoSjPreviewRequest && autoSjPreviewRequest.readyState !== 4) {
                    autoSjPreviewRequest.abort();
                }

                if (!idGudang || !tanggalSurat) {
                    setAutoSjPreview('Pilih area gudang dan tanggal surat untuk melihat nomor surat jalan berikutnya.', true);
                    return;
                }

                setAutoSjPreview('Memuat urutan nomor surat jalan berikutnya...', false);
                autoSjPreviewReady = false;

                autoSjPreviewRequest = $.ajax({
                    url: "<?= site_url('Dashboard_Logistik_Stok/getPreviewSuratJalanNumber') ?>",
                    type: "GET",
                    dataType: "json",
                    data: {
                        id_lokasi_gudang: idGudang,
                        tanggal_upload_stok: tanggalSurat
                    },
                    success: function(response) {
                        const nomorSurat = String(response.nomor_surat_jalan || '').trim();
                        const sequence = Number(response.nomor_surat_jalan_seq || 0);
                        if (!nomorSurat) {
                            setAutoSjPreview('Preview nomor surat jalan belum tersedia. Pastikan generator counter surat jalan sudah siap.', true);
                            checkNomorSuratJalan();
                            applyOutStockGuard();
                            return;
                        }

                        if (shouldUseManualAutoOutProjectFlow()) {
                            const $nomorSuratInput = $('#revamp_nomor_surat_jalan_text');
                            const currentValue = String($nomorSuratInput.val() || '').trim();
                            if (!currentValue || currentValue === autoSjLastSuggested) {
                                $nomorSuratInput.val(nomorSurat);
                                setNomorSuratJalanValue(nomorSurat);
                            }
                            autoSjLastSuggested = nomorSurat;
                        }

                        setAutoSjPreview(`Nomor surat jalan berikutnya yang akan dipakai: ${nomorSurat}${sequence > 0 ? ' (urutan ' + sequence + ')' : ''}`, false);
                        checkNomorSuratJalan();
                        applyOutStockGuard();
                    },
                    error: function(xhr, status) {
                        ...
                        setAutoSjPreview('Gagal memuat preview nomor surat jalan.', true);
                        checkNomorSuratJalan();
                        applyOutStockGuard();
                    }
                });
                ...
                gudangStockRequest = $.ajax({
                    ...
                    success: function(response) {
                        ...
                        $.each(response || [], function(_, item) {
                            if (isOutMaterialMode() && parseNumber(item.total_stok || 0) <= 0) {
                                return true;
                            }
            ...
            function loadHoReceiptItems() {
                ...
                $.ajax({
                    url: "<?= site_url('Dashboard_Logistik_Stok/getPengirimanHoBySuratJalan') ?>",
                    type: "GET",
                    dataType: "json",
                    data: {
                    ...
                    success: function(response) {
                        ...
                        rows.forEach(function(item, index) {
                            const qtyOutstanding = formatNumber(item.qty_outstanding_terima || 0);
                            const qtyKirim = formatNumber(item.qty_kirim || 0);
                            $tbody.append(`
                                <tr class="revamp-ho-row">
                                    <td>${index + 1}</td>
                                    <td>
                                        <input type="hidden" name="id_stock_pengiriman[${index}]" value="${sanitizeText(item.id_stock_pengiriman)}">
                                        ...
                                        <input type="hidden" name="qty_pengiriman_outstanding[${index}]" value="${sanitizeText(item.qty_outstanding_terima || 0)}">
                                        <strong>${sanitizeText(item.nama_item || '-')}</strong><br>
                                        <small class="text-muted">Asal ${sanitizeText(item.kota_gudang_asal || 'HO')} | PO ${sanitizeText(item.no_po_logistik || '-')} | Outstanding ${qtyOutstanding}</small>
                                    </td>
                                    <td><input type="text" class="form-control" value="${qtyKirim}" readonly disabled></td>
            ...
            function loadPabrikReceiptItems() {
                if (!isPabrikReceiptMode()) {
                    return;
                }
                ...
                $.ajax({
                    url: "<?= site_url('Dashboard_Logistik_Stok/getPengirimanPabrikBySuratJalan') ?>",
                    type: "GET",
                    dataType: "json",
                    data: {
                        nomor_surat_jalan: nomorSurat,
                    ...
                    success: function(response) {
                        ...
                        rows.forEach(function(item, index) {
                            ...
                            const qtyKirim = formatNumber(item.qty_kirim || 0);
                            $tbody.append(`
                                <tr class="revamp-pabrik-row">
                                    <td>${index + 1}</td>
                                    <td>
                                        <input type="hidden" name="id_pengiriman_pabrik_detail[${index}]" value="${sanitizeText(item.id_pengiriman_pabrik_detail)}">
                                        <input type="hidden" name="id_kode_item[${index}]" value="${sanitizeText(item.id_kode_item)}">
                                        <input type="hidden" name="id_bowheer_item[${index}]" value="${sanitizeText(item.id_bowheer_pemilik_item || '')}">
                                        <input type="hidden" name="qty_pengiriman_outstanding[${index}]" value="${sanitizeText(item.qty_outstanding_terima || 0)}">
                                        <strong>${sanitizeText(item.nama_item || '-')}</strong><br>
                                        <small class="text-muted">PO ${sanitizeText(item.nomor_po_pabrik || '-')} | PR ${sanitizeText(item.nomor_purchase_request || '-')} | Outstanding ${qtyOutstanding}</small>
                                    </td>
                                    <td><input type="text" class="form-control" value="${qtyKirim}" readonly disabled></td>
                                    <td><input type="text" class="form-control revamp-qty-input" name="jumlah_stok[${index}]" placeholder="0" value="${qtyOutstanding}" required></td>
                                    <td><input type="text" class="form-control revamp-qty-input revamp-qty-selisih-input" name="qty_selisih[${index}]" placeholder="0" value="0"></td>
                                    <td><input type="text" class="form-control" name="satuan_stok[${index}]" value="${sanitizeText(item.satuan_item || '')}" readonly></td>
                                    <td><input type="text" class="form-control" name="merk_item[${index}]" placeholder="Merk item"></td>
                                    <td><input type="text" class="form-control" name="no_haspel_item[${index}]" placeholder="No haspel"></td>
                                    <td><input type="text" class="form-control" name="no_ref_item[${index}]" placeholder="No ref"></td>
                                    <td><span class="text-muted">Auto</span></td>
                                </tr>
                            `);
                        });
            ...
            function refreshTambahTableHeader() {
                const isSpecialMode = isShipmentReceiptMode();
                $('#revamp_table_qty_primary_label').text(isSpecialMode ? 'Qty Pengiriman' : 'Qty');
                $('#revamp_table_qty_secondary_label').toggleClass('stock-hidden', !isSpecialMode);
            }
            ...
            $('#modalRevampTambahStok').on('hidden.bs.modal', function() {
                ...
                $('#revamp_confirm_sj_gap').val('0');
                resetHoSuratJalanOptions();
                $('#revamp_btn_submit_stok').prop('disabled', false);
                $('.custom-file-label').text('Choose file');
                $('#revamp_nomor_ba_selisih, #revamp_no_po_logistik').val('');
                $('#revamp_pabrik_sj_feedback').addClass('stock-hidden').text('Tidak ada surat jalan yang cocok.');
                $('#revamp_qty_validation_feedback').addClass('stock-hidden').text('');
                setAutoSjPreview('', false);
                autoSjLastSuggested = '';
                markNomorSuratJalanInvalid(false);
            ...
            $('#revamp_id_kode_item').on('change', function() {
                ...
                const shouldLimitOutStock = !isShipmentReceiptMode() && isOutMaterialMode();

                $('#revamp_table_item_stok tbody').append(`
                    <tr data-item-label="${sanitizeText(selectedText)}" data-available-stock="${selectedStock}">
            ...
            $('.custom-file-input').on('change', function() {
                ...
                if (!allowedExtensions.test(file.name)) {
                    Swal.fire({ icon: 'warning', title: 'Format file salah', text: 'File harus berupa PDF, JPG, JPEG, atau PNG.' });
                    $(this).val('');
            ...
            function validateManualOutProjectNomorSuratJalan() {
                return $.ajax({
                    url: "<?= site_url('Dashboard_Logistik_Stok/validateManualSuratJalanOutProject') ?>",
                    type: "POST",
                    dataType: "json",
                    data: {
                        id_lokasi_gudang: $('#revamp_id_lokasi_gudang').val(),
                        id_sumber_material: $('#revamp_id_sumber_material').val(),
                        tanggal_upload_stok: $('#revamp_tanggal_upload_stok').val(),
                        nomor_surat_jalan: getNomorSuratJalanValue()
                    }
                });
            }
            ...
            $('#revamp_form_tambah_stok').on('submit', function(event) {
                ...
                if (!isShipmentReceiptMode() && !$('#revamp_id_project').val()) {
                ...
                if (isAutoGeneratedSjMode() && !isShipmentReceiptMode() && !autoSjPreviewReady) {
                    errors.push('Preview nomor surat jalan belum siap. Cek area gudang, tanggal surat, atau generator counter.');
                }

                if (isPabrikReceiptMode()) {
                    let totalSelisih = 0;
                    let hasInvalidPabrikQty = false;

                    $('#revamp_table_item_stok tbody tr').each(function() {
                        const qtyTerima = parseNumber($(this).find('input[name^="jumlah_stok["]').val() || 0);
                        const qtySelisih = parseNumber($(this).find('input[name^="qty_selisih["]').val() || 0);
                        const qtyOutstanding = parseNumber($(this).find('input[name^="qty_pengiriman_outstanding["]').val() || 0);

                        totalSelisih += qtySelisih;
                        if ((qtyTerima + qtySelisih) <= 0 || (qtyTerima + qtySelisih) > qtyOutstanding) {
                            hasInvalidPabrikQty = true;
                        }
                    });

                    if (hasInvalidPabrikQty) {
                        errors.push('Qty terima + qty selisih tiap item harus lebih dari 0 dan tidak boleh melebihi outstanding pengiriman.');
                    }

                    if (totalSelisih > 0 && !$('#revamp_nomor_ba_selisih').val().trim()) {
                        errors.push('Nomor BA selisih wajib diisi jika ada material rusak / selisih.');
                    }
                }
                ...
                if (isOutMaterialMode() && !isShipmentReceiptMode() && !isMitraReturnMode()) {
                    const outProjectionState = getOutStockProjectionState();
                    if (outProjectionState.hasNegativeStock) {
                        errors.push(`Qty out material membuat stok minus.<br>${outProjectionState.messages.join('<br>')}`);
                    }
                }
                ...
                if (isOutProjectMode() && !$('#revamp_nama_mitra').val().trim()) {
                    errors.push('PIC Pengambil wajib diisi untuk Out (ke Project).');
                }
                if (isOutProjectMode() && !$('#revamp_pic_mitra').val().trim()) {
                    errors.push('Telp Pengambil wajib diisi untuk Out (ke Project).');
                }
                if (isOutProjectMode() && !$('#revamp_nomor_polisi').val().trim()) {
                    errors.push('NOPOL Kendaraan wajib diisi untuk Out (ke Project).');
                }
                ...
                if (isHoReceiptMode()) {
                    ...
                    $('#revamp_table_item_stok tbody tr').each(function() {
                        ...
                        const qtySelisih = parseNumber($(this).find('input[name^="qty_selisih["]').val() || 0);
                        const qtyOutstanding = parseNumber($(this).find('input[name^="qty_pengiriman_outstanding["]').val() || 0);
                        totalSelisihHo += qtySelisih;
                        if (qtyTerima <= 0 || qtyTerima > qtyOutstanding) {
                            hasInvalidHoQty = true;
                        }
                    });

                    if (hasInvalidHoQty) {
                        errors.push('Qty diterima dari HO harus lebih dari 0 dan tidak boleh melebihi outstanding pengiriman.');
                    }

                    if (totalSelisihHo > 0) {
                        errors.push('Selisih untuk penerimaan dari HO belum didukung pada flow transit ini.');
                    }
                }
                ...
                if (shouldUseManualAutoOutProjectFlow()) {
                    event.preventDefault();
                    validateManualOutProjectNomorSuratJalan()
                        .done(function(response) {
                            ...
                            if (status === 'exists') {
                                Swal.fire({
                                    ...
                                    text: 'Nomor tersebut sudah dipakai.'
                                });
                                return;
                            }
                            ...
                            if (status === 'invalid') {
                                Swal.fire({
                                    ...
                                    text: response.message || 'Format nomor surat jalan tidak valid.'
                                });
                                return;
                            }
                            ...
                            if (status === 'gap') {
                                Swal.fire({
                                    icon: 'question',
                                    title: 'Konfirmasi nomor loncat',
                                    text: response.message || 'Ada nomor sebelumnya yang belum dipakai. Lanjutkan?',
                            ...
                            Swal.fire({
                                ...
                                text: response.message || 'Terjadi kesalahan saat validasi nomor surat jalan.'
                            });
                        })
                        .fail(function() {
                            Swal.fire({
                                icon: 'error',
                                title: 'Validasi gagal',
                                text: 'Tidak dapat memvalidasi nomor surat jalan saat ini. Silakan coba lagi.'
                            });
                        });
                }
            });
            ...
            $('#revamp_tanggal_upload_stok').on('change', function() {
                loadAutoGeneratedSjPreview();
                applyOutStockGuard();
            });

            $('#revamp_table_item_stok').on('input', '.revamp-qty-selisih-input', function() {
                const value = $(this).val().replace(/[^\d]/g, '');
                $(this).val(value.replace(/\B(?=(\d{3})+(?!\d))/g, '.'));
            });
        ...
        function toggleHoFields() {
            ...
            const isSpecialMode = isPabrikMode || isHoMode || isMitraReturn;
                ...
                const $nomorSuratPreview = $('#revamp_nomor_surat_jalan_preview');
            ...
            const showPengambilFields = isOutProjectMode() || (currentRule && Number(currentRule.require_nomor_polisi || 0) === 1) || (currentRule && Number(currentRule.require_nama_mitra || 0) === 1) || (currentRule && Number(currentRule.require_pic_mitra || 0) === 1);
            $('#revamp_nomor_polisi_wrapper, #revamp_nama_mitra_wrapper, #revamp_pic_mitra_wrapper').toggleClass('stock-hidden', !showPengambilFields);
            $('#revamp_ho_out_estimasi_sampai_wrapper, #revamp_ho_out_nama_ekspedisi_wrapper, #revamp_ho_out_pic_ekspedisi_wrapper')
                ...
                $nomorSuratPreview.toggleClass('stock-hidden', !(ruleMode === 'AUTO' || ruleMode === 'AUTO_WITH_REFERENCE'));
                ...
                if (ruleMode === 'AUTO' && referenceMode === 'NONE' && !isSpecialMode && !isOutProjectMode()) {
                    $nomorSuratText.val('');
                    setNomorSuratJalanValue('');
                    $nomorSuratText.prop('readonly', true).attr('placeholder', 'Otomatis generate saat simpan');
                } else if (shouldUseManualAutoOutProjectFlow()) {
                    $nomorSuratText.prop('readonly', false).attr('placeholder', 'Contoh: TEC.003/TKM-01/SJ/V/2026');
                }

                if (isOutProjectMode()) {
                    $('label[for="revamp_nama_mitra"]').text('PIC Pengambil');
                    $('label[for="revamp_pic_mitra"]').text('Telp Pengambil');
                    $('label[for="revamp_nomor_polisi"]').text('NOPOL Kendaraan');
                    $('#revamp_nama_mitra').attr('placeholder', 'Nama PIC pengambil');
                    $('#revamp_pic_mitra').attr('placeholder', '08xxxxxxxxxx');
                    $('#revamp_nomor_polisi').attr('placeholder', 'B 1234 TKM');
                    $('#revamp_nama_mitra, #revamp_pic_mitra, #revamp_nomor_polisi').prop('required', true);
                } else {
                    $('label[for="revamp_nama_mitra"]').text('Nama Mitra');
                    $('label[for="revamp_pic_mitra"]').text('PIC Mitra');
                    $('label[for="revamp_nomor_polisi"]').text('Nomor Polisi');
                    $('#revamp_nama_mitra').attr('placeholder', 'Nama mitra');
                    $('#revamp_pic_mitra').attr('placeholder', 'Nama PIC mitra');
                    $('#revamp_nomor_polisi').attr('placeholder', 'B 1234 TKM');
                    $('#revamp_nama_mitra, #revamp_pic_mitra, #revamp_nomor_polisi').prop('required', false);
                }
        ...
        function checkNomorSuratJalan() {
            if (isAutoGeneratedSjMode() && !isShipmentReceiptMode()) {
                ...
                markNomorSuratJalanInvalid(false);
                $('#revamp_pabrik_sj_feedback').addClass('stock-hidden').text('Tidak ada surat jalan yang cocok.');
                $('#revamp_btn_submit_stok').prop('disabled', !autoSjPreviewReady);
                applyOutStockGuard();
                return;
            }
            ...
            $(document).on('click', '.revamp-detail-trigger', function() {
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        $('#revamp_detail_nama_mitra').val(first.nama_mitra || '');
                        $('#revamp_detail_pic_mitra').val(first.pic_mitra || '');
                        $('#revamp_detail_nomor_polisi').val(first.nomor_polisi || '');
                        ...
                        const sourceMaterialId = String(first.id_sumber_material || '');
                        const hasOutProjectAuditMeta = Boolean(first.nama_mitra || first.pic_mitra || first.nomor_polisi);
                        const showOutProjectAuditMeta = sourceMaterialId === '9' || hasOutProjectAuditMeta;
                        $('#revamp_detail_pic_pengambil_wrapper, #revamp_detail_telp_pengambil_wrapper, #revamp_detail_nopol_wrapper')
                            .toggleClass('stock-hidden', !showOutProjectAuditMeta);
