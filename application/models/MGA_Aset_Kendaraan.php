...
class MGA_Aset_Kendaraan extends CI_Model
{
    ...
    SUM(CASE WHEN tb_aset_kendaraan.ak_kondisi_aset = "BAIK" && tb_aset_kendaraan.ak_status_aset = "AKTIF" THEN 1 ELSE 0 END) AS jml_baik,
    SUM(CASE WHEN tb_aset_kendaraan.ak_kondisi_aset = "RUSAK" && tb_aset_kendaraan.ak_status_aset = "AKTIF" THEN 1 ELSE 0 END) AS jml_rusak,
...
ORDER BY tb_aset_kendaraan.ak_area ASC;')
    ...
    public function tambahKendaraan($data_array)
    {
        $res = $this->db->insert("tb_aset_kendaraan", $data_array);
        return $res;
    }

    public function hapusKendaraan($ak_id_list_kendaraan)
    {
        $res = $this->db->delete("tb_aset_kendaraan", $ak_id_list_kendaraan);
        return $res;
    }
